...
void RecebeVerificaV () {
	//Pegando a Primeira declaração: G = (N,T,P,S)
	scanf(" %c = ( %c , %c , %c , %c )",&simboloGramatica, &simboloVariaveis, &simboloTerminais,&simboloDerivacoes,&variavelInicial);

	//Verifique se as variáveis ​​são definidas ou terminais.
	char firstChar, aux;
	fore(i,0,2){
		scanf(" %c = ",&firstChar);
...
void ContruirRecebimentoDados ( vector < vector <string> > & derivacoes,char variavel, string valores ){
						...
						if ( mapaVariaveis.find(palavra[k+1]) != mapaVariaveis.end() ){
							...
							while ( fim == false ){
								...
								if ( mapaVariaveis.find(palavra[n]) == mapaVariaveis.end() ){
									...
									//Verificar se no conjunto first da variável seguinte tem o vazio que é representado por &.
...
void ImprimirConjunto ( vector < set<char> > & conjunto, string tag ){
	set <char> :: iterator itset;
...
void Gramatica (){
	...
	while ( cin >> simbol){
		...
		scanf(" -> ");
		getline(cin, aux);
		ContruirRecebimentoDados(derivacoes,simbol,aux);
	}

	conjuntoFirst = first(derivacoes);
ImprimirConjunto(conjuntoFirst,"First");
...
ImprimirConjunto(conjuntoFollow,"Follow");
